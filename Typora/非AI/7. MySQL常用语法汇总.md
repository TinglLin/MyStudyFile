...
- 原子性、一致性、隔离性、持久性
  - 原子性：事务是最小执行单位，所有操作要么全部提交成功，要么全部回滚失败
  - 一致性：事务执行前后，数据库的完整性约束不变，数据始终合法正确
  - 隔离性：多个并发事务之间相互隔离、互不打扰
  - 持久性：事务一旦提交，数据永久保存到磁盘



#### 3.2 并发事务会出现的3大问题

- **脏读：**读到其他事务未提交的数据（对方可能回滚）
- **不可重复读：**同一事务内，两次查询结果不一样（被其他事务修改并提交）
- **幻读：**同一事务内，查询到新增/删除的记录，结果集数量变了



#### 3.3 事务的4大隔离级别，解决了什么问题？带来了什么？

- **读未提交 Read Uncommitted**
  - 能读到别的事务没提交的数据
  - 解决：无
  - 存在问题：脏读、不可重复读、幻读（全部都有）
...
- **可重复读 Repeatable Read**
  - 事务启动时，就“定格”数据，全程查询结果一致
  - 解决：脏读、不可重复读
  - 存在问题：幻读（MySQL靠MVCC已很大程度缓解）
  - MySQL默认
...
| 读未提交 | ✅ 会   | ✅ 会       | ✅ 会               | 不安全         |
| 读已提交 | ❌ 不会 | ✅ 会       | ✅ 会               | 常用           |
| 可重复读 | ❌ 不会 | ❌ 不会     | ✅ 会（MySQL 缓解） | **MySQL 默认** |
| 串行化   | ❌ 不会 | ❌ 不会     | ❌ 不会             | 性能差         |
